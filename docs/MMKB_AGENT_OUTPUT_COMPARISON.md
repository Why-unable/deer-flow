...
- 请求进入 Agent 代理后的即时准备状态；
- 本轮采用的 skill 名称；
- RAG 检索请求与结果数量；
- 文档信息、预览、片段、图片和集合读取；
- 网页和图片搜索状态；
- 文件读取、写入、目录查看和修改的开始与完成状态；
- Bash 命令的开始与完成状态；
- Todo 计划与状态；
- 子任务开始、阶段进度、完成和异常；
- 文件准备状态；
...
| 子 Agent 实时过程 | 支持 | 仅显示安全的阶段序号进度，不展示内部正文 |
...
MMKB 会把 `task_running` 中每个新 AI 消息转换成安全、去重的“已推进到第 N
个阶段”反馈，但不会展示其正文、内部工具过程或完整结果卡片。
