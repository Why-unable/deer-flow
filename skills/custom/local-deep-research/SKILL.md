...
图片资产硬预算（必须遵守，防止逐张狂拉图片耗尽步数、拖垮回答）：

- **只对已确定要放进回答的图**调用 `rag_get_document_asset`，不要"先全拉再挑"。
- 单次回答 `rag_get_document_asset` 全程 ≤ 6 次；`rag_get_document_assets` 最多翻 2-3 页。
- 多数问题不需要图，文字证据足够时直接引用文字，不取图。
- 系统在引擎层对取图工具设了强制上限（超过约 12 次自动收尾），主动遵守本预算即可。

回答必须产出：一旦证据足够，立即综合成最终回答；**禁止**用"让我现在生成回答/报告"
这类过渡语作为结束——过渡语后必须紧跟真正的内容。若用 subagent 且其失败或中间文件缺失，
主 agent 直接用已有证据作答，并说明缺失部分，绝不因子任务没跑完就不给回答。
