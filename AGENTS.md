...
## MMKB Integration Overlay

This branch is not a plain upstream checkout. It contains the MMKB integration
documented in `docs/MMKB_INTEGRATION_CHANGES.md`; that document is the source of
truth for the branch-specific inventory. Also read `docs/MMKB_URL_LIFECYCLE.md`
before changing resource links and
`docs/MMKB_AGENT_STORAGE_AND_OOM_TROUBLESHOOTING.md` before changing run-event
storage or long-running SSE behavior.

Preserve these invariants when developing or syncing upstream:

- MMKB authenticates its users and enforces workspace document access.
  DeerFlow treats `mmkb_bearer_token` as opaque and replays it only to MMKB.
- The MMKB proxy bypass is limited to an internally authenticated run-create
  request carrying the opaque bearer plus trusted workspace/user context.
- Thread, artifact, and memory ownership is isolated by MMKB workspace + user.
- Delegated subagents receive only the explicit MMKB bearer/public-URL/identity
  allowlist, never the whole parent runtime configuration.
- `tools.py` owns LangChain schemas; `client.py` owns HTTP transport;
  `service.py`, `assets.py`, and `mmkb_links.py` own reusable response policy.
- Tool HTTP failures retain the established JSON error contract.
- MMKB signs protected media. DeerFlow only absolutizes links and emits
  count-only diagnostics; logs must not contain bearer tokens, signed tokens,
  document IDs, private paths, or complete resource URLs.

Focused regression tests live in `backend/tests/test_mmkb_*.py`, with related
coverage in `test_gateway_services.py`, `test_run_journal.py`, memory tests, and
subagent tests. After an upstream sync, verify run creation, delegated RAG,
resource URLs, artifact ownership, request-scoped tenant models, and
workspace+user memory isolation. Update `docs/MMKB_INTEGRATION_CHANGES.md` when
any of those behaviors or their implementation files change.
