...
_SUBAGENT_MMKB_CONFIGURABLE_KEYS = ("mmkb_bearer_token", "public_base_url")
_SUBAGENT_MMKB_CONTEXT_KEYS = (
    "public_base_url",
    "mmkb_workspace_id",
    "mmkb_user_id",
    "mmkb_tenant_id",
...
def _get_subagent_mmkb_runtime_values(runtime: Runtime | None) -> tuple[dict[str, Any], dict[str, Any]]:
    """Copy only the MMKB values that delegated RAG tools require.

    Parent run mappings can also contain unrelated provider credentials and
    server-owned authorization state, so delegation deliberately uses this
    narrow allowlist instead of forwarding those mappings wholesale.
    """
    if runtime is None:
        return {}, {}
    parent_context = runtime.context if isinstance(runtime.context, dict) else {}
    parent_config = runtime.config if isinstance(runtime.config, dict) else {}
    parent_configurable = parent_config.get("configurable")
    parent_configurable = parent_configurable if isinstance(parent_configurable, dict) else {}
    configurable = {key: value for key in _SUBAGENT_MMKB_CONFIGURABLE_KEYS if (value := parent_configurable.get(key) or parent_context.get(key)) is not None}
    context = {key: value for key in _SUBAGENT_MMKB_CONTEXT_KEYS if (value := parent_context.get(key) or parent_configurable.get(key)) is not None}
...
async def task_tool(
    ...
    inherited_mmkb_configurable, inherited_mmkb_context = _get_subagent_mmkb_runtime_values(runtime)
    ...
    executor_kwargs = {
        ...
        "inherited_mmkb_configurable": inherited_mmkb_configurable,
        "inherited_mmkb_context": inherited_mmkb_context,
    }
    if resolved_app_config is not None:
