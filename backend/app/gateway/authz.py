...
def _is_mmkb_proxy_run_create(resource: str, action: str, kwargs: dict[str, Any]) -> bool:
    """Recognize the narrow, internally authenticated MMKB run-create hop.

    MMKB authenticates the external user and forwards an opaque workspace
    bearer for the RAG tools.  DeerFlow never parses that bearer.  Requiring
    the shared internal token here prevents arbitrary clients from obtaining
    the run-create bypass merely by inventing a configurable key.
    """
    ...
    context = _body_mapping_value(body, "context")
    if not isinstance(config, dict) or not isinstance(context, dict):
        return False
    configurable = config.get("configurable")
    if not isinstance(configurable, dict) or not str(configurable.get("mmkb_bearer_token") or "").strip():
        return False
    return bool(str(context.get("mmkb_workspace_id") or "").strip() and str(context.get("mmkb_user_id") or "").strip())
