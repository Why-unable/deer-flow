...
def _mmkb_runtime_model_config(context: Mapping[str, Any] | None) -> ModelConfig | None:
    """Convert one MMKB tenant model into a request-scoped DeerFlow model."""
    if not isinstance(context, Mapping):
        return None
    raw = context.get("mmkb_model_config")
    if not isinstance(raw, Mapping):
        return None
    if str(raw.get("protocol") or "openai_compatible").strip().lower() != "openai_compatible":
        return None
    ...
    return ModelConfig(
        ...
        supports_thinking=raw.get("supports_thinking") is True,
        supports_reasoning_effort=raw.get("supports_reasoning_effort") is True,
        supports_vision=raw.get("supports_vision") is True,
        context_window=_optional_int(raw.get("context_window"), 0) or None,
        stream_chunk_timeout=_optional_float(raw.get("stream_chunk_timeout"), 0.0) or None,
    )


def _app_config_with_mmkb_runtime_model(base_config: AppConfig, runtime_model: ModelConfig | None) -> AppConfig:
    """Return a per-run AppConfig copy containing the MMKB tenant model."""
    if runtime_model is None:
        return base_config
    # Re-validate instead of assigning to ``model_copy().models``: current
    # AppConfig maintains private O(1) name indexes that are only rebuilt by
    # its model validator.  A plain list assignment would make the model
    # visible in ``.models`` but invisible to ``get_model_config``.
    config_data = base_config.model_dump()
    config_data["models"] = [
        runtime_model.model_dump(),
        *[model.model_dump() for model in base_config.models if model.name != runtime_model.name],
    ]
    return AppConfig.model_validate(config_data)


def _context_without_mmkb_model_config(context: Mapping[str, Any] | None) -> Mapping[str, Any] | None:
    """Keep the tenant API key out of RunnableConfig and checkpoints."""
...
def resolve_mmkb_proxy_user(request: Request, context: Mapping[str, Any] | None) -> SimpleNamespace | None:
    """Map an admitted MMKB workspace/user pair to one DeerFlow owner bucket."""
    if not getattr(request.state, "mmkb_proxy", False) or not isinstance(context, Mapping):
        return None
    workspace_id = str(context.get("mmkb_workspace_id") or "").strip()
    user_id = str(context.get("mmkb_user_id") or "").strip()
    if not workspace_id or not user_id:
        return None
    effective_id = make_safe_user_id(f"mmkb-{workspace_id}-{user_id}")
    return SimpleNamespace(id=effective_id, system_role="mmkb_proxy")
...
def merge_run_context_overrides(config: dict[str, Any], context: Mapping[str, Any] | None, *, internal: bool = False) -> None:
    ...
    thread_id = str(configurable.get("thread_id") or "") if isinstance(configurable, dict) else ""
    logger.info(
        "deerflow_run_context_merge thread_id=%s incoming_public_base_url_present=%s incoming_public_base_url=%s configurable_public_base_url=%s context_public_base_url=%s mmkb_workspace_present=%s mmkb_user_present=%s",
...
async def start_run(
    ...
    *,
    ...
    mmkb_app_config = None
    if mmkb_runtime_model is not None:
        mmkb_app_config = _app_config_with_mmkb_runtime_model(run_ctx.app_config or get_app_config(), mmkb_runtime_model)
    ...
    if model_name:
        app_config = mmkb_app_config or get_app_config()
    ...
    owner_user_id = str(mmkb_user.id) if mmkb_user is not None else get_trusted_internal_owner_user_id(request)
    ...
    try:
        ...
        merge_run_context_overrides(
            config,
            _context_without_mmkb_model_config(body_context),
        ...
        effective_run_ctx = replace(run_ctx, app_config=mmkb_app_config) if mmkb_app_config is not None else run_ctx
