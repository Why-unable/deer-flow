...
VALID_SIGNED_URL = "http://localhost:8000/api/document-media/.eJyrVkrLz1eyUkpKLFKqBQAqYgWJ%3A1wXgTO%3AJFZy4Ixr9YrBXiUSAVb_JHplVN8Ueh9jsglBgaO3VM4"
MALFORMED_SIGNED_URL = "http://localhost:8000/api/document-media/.eJwlYzEKgkAQhT9ItFjUGawQahVBEJJXq00lKa9t1c2d3vfvtxL48h8Yk0I50t9p27uPZ2Vgmk5Z0o0E0KxRQrVxRqtFqN4lJcY9VvQ%3D"
...
class TestLlmCallbacks:
    ...
    async def test_on_llm_end_records_final_assistant_resource_validation(self, journal_setup):
        ...
        events = await store.list_events("t1", "r1")
        validation = [event for event in events if event["event_type"] == "assistant.resource.validation"]
        assert len(validation) == 1
        assert validation[0]["category"] == "trace"
        assert validation[0]["content"]["resource_urls"] == 2
        assert validation[0]["content"]["structurally_valid_signed"] == 1
        assert validation[0]["content"]["malformed_signed"] == 1

    @pytest.mark.anyio
    async def test_on_llm_end_validates_document_origin(self):
        store = MemoryRunEventStore()
        journal = RunJournal(
            "r1",
            "t1",
    ...
    @pytest.mark.anyio
    async def test_on_llm_end_skips_validation_for_tool_calls_and_subagents(self, journal_setup):
        j, store = journal_setup

        j.on_llm_end(
            _make_llm_response(
                f"![pending]({MALFORMED_SIGNED_URL})",
        ...
        j.on_llm_end(
            _make_llm_response(f"![subagent]({MALFORMED_SIGNED_URL})"),
            run_id=uuid4(),
            parent_run_id=None,
