...
## MMKB-Specific Backend Boundaries

MMKB calls the thread run stream endpoint through the shared internal token and
passes an opaque workspace bearer for the eight custom RAG tools under
`packages/harness/deerflow/tools/custom/rag/`. Gateway admission and identity
mapping stay in `app/gateway/`; the reusable RAG client/service/tool stack stays
in the harness and must not import `app.*`.

`services.start_run()` maps trusted MMKB workspace/user context to a combined
owner bucket and may build a request-scoped `AppConfig` copy for a tenant model.
The tenant API key must not enter RunnableConfig, checkpoints, run metadata, or
logs. `task_tool` and `SubagentExecutor` propagate only the explicit MMKB
allowlist needed by delegated RAG tools. `RunJournal` performs structure-only
validation of final MMKB links; MMKB remains responsible for signing,
authorization, expiry, and existence.
