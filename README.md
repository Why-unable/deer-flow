...
For local MMKB integration, MMKB can call the LangGraph-compatible thread run stream endpoint after authenticating its own user. It forwards its opaque `Bearer workspace_id:secret` token as `config.configurable.mmkb_bearer_token` and sends `context.public_base_url`; DeerFlow passes these values through to the custom RAG tools so they can call MMKB APIs and absolutize returned asset URLs. When the lead agent delegates research to a subagent, DeerFlow also carries this narrow MMKB authentication and identity context into the delegated run so its RAG tools remain workspace-scoped. The RAG integration separates LangChain tool schemas, runtime context, a transport-neutral semantic MMKB client, shared response/link policy, and asset projection; HTTP is the current client implementation, while future transports such as MCP can reuse the same service and policy layers. Ordinary topical questions use `rag_search` as the default entry point, while `rag_list_documents` is reserved for document inventory, scope discovery, and multi-document review candidate pools. Visual assets use a two-stage flow: `rag_get_document_assets` returns a bounded, paginated compact catalog with complete signed URLs, and `rag_get_document_asset` returns a leading URL manifest plus one selected asset's full OCR and metadata; `rag_get_document_preview` strips raw Markdown image paths and `IMG_META` comments so displayable images must come from asset `image_url` fields. Each RAG tool response also emits count-only `mmkb_link_stage4` and `mmkb_tool_resource_validation` logs. Final lead-agent answers containing MMKB resource URLs emit an `assistant_resource_validation` log and a persisted `assistant.resource.validation` run event. These checks cover signed media structure plus ordinary `/documents/<uuid>` page shape and expected origin; document pages remain intentionally dependent on a browser session and matching current workspace.
