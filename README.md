...
## MMKB Knowledge-Base Integration

This branch lets MMKB use DeerFlow as its Agent runtime. MMKB authenticates the
caller, sends a shared internal Gateway token, and forwards an opaque
workspace-scoped bearer to DeerFlow's eight `rag_*` tools. The tools provide
document inventory, hybrid knowledge retrieval, document/chunk access, and a
bounded two-stage visual-asset workflow while preserving signed user-visible
resource URLs.

MMKB requests are isolated by workspace + user across threads, artifacts, and
memory. The same narrow identity and bearer context is carried into delegated
subagents, and a tenant-selected OpenAI-compatible model is installed only in
the current run's configuration copy. See
[`docs/MMKB_INTEGRATION_CHANGES.md`](docs/MMKB_INTEGRATION_CHANGES.md) for the
implementation inventory and
[`docs/MMKB_URL_LIFECYCLE.md`](docs/MMKB_URL_LIFECYCLE.md) for link delivery.
